...
/// Magic bytes at the start of every ne_admin1.bin file.
const _kMagic = [0x52, 0x4C, 0x52, 0x47]; // "RLRG"
const _kVersion = 1;

/// A single admin1 polygon extracted from the binary asset.
class RegionPolygon {
  /// ISO 3166-2 region code, e.g. "US-CA", "GB-ENG", "FR-IDF".
  final String regionCode;

  /// Polygon vertices as (lat, lng) pairs in decimal degrees.
  final List<(double, double)> vertices;

  const RegionPolygon({required this.regionCode, required this.vertices});
}

/// Parsed representation of the ne_admin1.bin spatial index.
///
...
///   For each polygon:
///     [0]     code_len    — uint8, number of ASCII bytes in the region code
///     [1..N]  code        — code_len ASCII bytes (ISO 3166-2, e.g. "US-CA")
///     [N+1..N+2] vertex_count — uint16
...
class RegionGeodataIndex {
  ...
  final List<RegionPolygon> _polygons;

  RegionGeodataIndex._({
    ...
    required List<RegionPolygon> polygons,
  ...
  factory RegionGeodataIndex.parse(Uint8List bytes) {
    ...
    // Read polygon data (variable-length region codes).
    final polygons = <RegionPolygon>[];
    for (var i = 0; i < polyCount; i++) {
      final codeLen = data.getUint8(offset++);
      final codeBytes = <int>[];
      for (var c = 0; c < codeLen; c++) {
        codeBytes.add(data.getUint8(offset++));
      }
      final regionCode = String.fromCharCodes(codeBytes);
      ...
      polygons.add(RegionPolygon(regionCode: regionCode, vertices: vertices));
    }

    return RegionGeodataIndex._(
  ...
  List<RegionPolygon> get polygons => List.unmodifiable(_polygons);

  /// Returns all polygons whose bounding-box grid cells overlap ([lat], [lng]).
  List<RegionPolygon> candidatesAt(double lat, double lng) {
    final col =
