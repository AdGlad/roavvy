...
/// yield the same [DesignRecipe]. (The renderer stays a pure function of the
/// recipe and consumes no entropy of its own.)
///
/// Split the master stream into **named sub-streams** (e.g. `'family'`,
/// `'layout'`, `'palette'`, `'torn:top'`) with [stream] so that adding a new
/// random draw in one dimension does not shift the numbers consumed by every
/// other dimension. Sub-stream names are part of the determinism contract:
/// treat them as append-only.
///
/// Ported verbatim (behaviour-identical) from the shipping engine's
/// `deterministic_rng.dart` so recipes stay reproducible across the boundary.
