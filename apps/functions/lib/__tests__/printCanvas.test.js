...
describe('printDimensions — t-shirt canvas', () => {
    const dims = printDimensions_1.PRINT_DIMENSIONS[TSHIRT_GID];
    ...
    it('all 25 t-shirt variants share the same dimensions', () => {
        const tshirtGids = Object.keys(printDimensions_1.PRINT_DIMENSIONS).filter((gid) => printDimensions_1.PRINT_DIMENSIONS[gid].dpi === 150 && printDimensions_1.PRINT_DIMENSIONS[gid].backgroundColor === 'transparent');
        expect(tshirtGids.length).toBe(25);
        for (const gid of tshirtGids) {
            const d = printDimensions_1.PRINT_DIMENSIONS[gid];
...
describe('printDimensions — poster canvas', () => {
    it('poster variants are unchanged (300 DPI, white background)', () => {
        const d = printDimensions_1.PRINT_DIMENSIONS[POSTER_GID];
...
//# sourceMappingURL=printCanvas.test.js.map