...
void main() {
  ...
  test('empty list returns a valid world-level title', () async {
    ...
    expect(result.title, isIn(['World Tour', 'Everywhere', 'Global Wander']));
  ...
  test(
    '{NO, SE, FI, IS, DK} → one of the Nordic options (full cluster)',
    () async {
      final result = await gen().generate(
        _req(codes: ['NO', 'SE', 'FI', 'IS', 'DK']),
      );
      expect(
        result.title,
        isIn(['Nordic Wander', 'Northern Lights', 'Fjord Life']),
  ...
  test('{EE, LV, LT} → one of the Baltic Loop options', () async {
    ...
    expect(result.title, isIn(['Baltic Loop', 'Baltic Run', 'Coast to Coast']));
  });
  ...
  test('mixed continents returns non-empty title', () async {
    final result = await gen().generate(_req(codes: ['JP', 'BR', 'US', 'DE']));
  ...
  test('dominant Asia continent → one of the Asia options', () async {
    ...
    final result = await gen().generate(_req(codes: ['IN', 'PK', 'BD', 'DE']));
    expect(
      result.title,
  ...
  test(
    'repeated calls for the same input produce more than one distinct title',
    () async {
  ...
  group('label-based titles', () {
    ...
    test(
      'mountain + golden_hour → one of the Alpine Gold combo options',
      () async {
        final result = await gen().generate(
          _req(
            codes: ['CH'],
    ...
    test(
      'mountain + hiking activity → one of the Trail Blazer options',
      () async {
        final result = await gen().generate(
          _req(
            codes: ['NO'],
    ...
    test('city scene only → one of the Urban Escape solo options', () async {
      final result = await gen().generate(
        _req(
          codes: ['DE'],
          heroLabels: [const HeroLabels(primaryScene: 'city', confidence: 0.6)],
        ),
      );
      expect(
        result.title,
