...
void main() {
  ...
  testWidgets(
    'Direction shows 6 subjects; Detail is Flags-only + Next skips it',
    (tester) async {
      final c = controller();
  ...
  testWidgets(
    'silhouette picker selects a specific one; it renders + persists',
    (tester) async {
      final c = controller();
      addTearDown(c.dispose);
      ...
      await tester.pump();
      expect(
        find.byKey(const Key('v2-detail-silhouette-pick')),
        findsOneWidget,
      );
      ...
      expect(
        find.byKey(const Key('v2-silhouette-fr_eiffel_tower')),
        findsOneWidget,
      );
