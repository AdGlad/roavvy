...
void main() {
  ...
  group('FirestoreSyncService.flushDirty', () {
    ...
    test('writes user-added country to Firestore and marks it clean', () async {
      ...
      await repo.saveAdded(
        UserAddedCountry(countryCode: 'JP', addedAt: DateTime.utc(2024, 1, 15)),
      );
    ...
    test(
      'writes user-removed country to Firestore and marks it clean',
      () async {
        final fakeFirestore = FakeFirebaseFirestore();
        final service = FirestoreSyncService(fakeFirestore);
        final repo = _makeRepo();
    ...
    test(
      'no GPS coordinates, filenames, or PHAsset ids in Firestore documents',
      () async {
        final fakeFirestore = FakeFirebaseFirestore();
        final service = FirestoreSyncService(fakeFirestore);
        final repo = _makeRepo();
  ...
  group('FirestoreSyncService field guard — no GPS or photo data written', () {
    ...
    test(
      'inferred visit document contains no photo identifier fields',
      () async {
        await repo.saveInferred(
          InferredCountryVisit(
            countryCode: 'FR',
    ...
    test('user_added document contains only permitted fields', () async {
      await repo.saveAdded(
        UserAddedCountry(countryCode: 'JP', addedAt: DateTime.utc(2024, 1, 1)),
      );
  ...
  group('FirestoreSyncService — achievement sync', () {
    test(
      'flushDirty with achievementRepo writes achievement to Firestore',
      () async {
        final fakeFirestore = FakeFirebaseFirestore();
        final service = FirestoreSyncService(fakeFirestore);
        final db = _makeDb();
    ...
    test(
      'achievement is marked clean in Drift after Firestore write',
      () async {
        final fakeFirestore = FakeFirebaseFirestore();
        final service = FirestoreSyncService(fakeFirestore);
        final db = _makeDb();
    ...
    test(
      'unlocking already-unlocked achievement is idempotent (no duplicate row)',
      () async {
        final db = _makeDb();
        final achievementRepo = AchievementRepository(db);
