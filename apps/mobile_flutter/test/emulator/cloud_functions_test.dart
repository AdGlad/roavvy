...
void main() {
  ...
  group('T6.9 — Client reads MerchConfig status from Firestore', () {
    test('cartItem status is mockupGenerating before function completes', () async {
    ...
    test('cartItem transitions to mockupReady after function writes URLs', () async {
      final fs = FakeFirebaseFirestore();
      final repo = MerchCartRepository(fs);
      const uid = 'user-001';
  ...
  group('T6.10 — Client handles function failure', () {
    test('cartItem transitions to failed when markFailed is called', () async {
      ...
      await repo.markFailed(uid, 'item-fn-3',
      ...
      expect(doc.data()!['failureReason'],
          'Printful returned 422: Invalid placement');
    });

    test('failed item remains in loadActive (not purchased — still visible)', () async {
      final fs = FakeFirebaseFirestore();
      final repo = MerchCartRepository(fs);
      const uid = 'user-001';
    ...
    test('failure reason is recorded in Firestore document', () async {
      ...
      final now = DateTime.utc(2026, 1, 1);
      await repo.create(uid, MerchCartItem(
        id: 'item-fn-5',
        status: MerchCartItemStatus.mockupGenerating,
        productType: 'tshirt',
        variantId: 'gid://shopify/ProductVariant/99',
