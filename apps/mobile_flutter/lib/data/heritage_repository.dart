...
class HeritageRepository {
        ...
        final merged =
            prev == null
                ...
                : VisitedHeritageSitesCompanion.insert(
                  ...
                  inscriptionYear: Value(site.inscriptionYear),
                  firstSeen:
                      prev.firstSeen.isBefore(site.firstSeen)
                          ? prev.firstSeen
                          : site.firstSeen.toUtc(),
                  lastSeen:
                      prev.lastSeen.isAfter(site.lastSeen)
                          ? prev.lastSeen
                          : site.lastSeen.toUtc(),
                  photoCount: Value(prev.photoCount + site.photoCount),
                  confidence: _strongerConfidence(
                    prev.confidence,
                    site.confidence,
                  ),
                  nearestDistanceKm: min(
                    prev.nearestDistanceKm,
                    site.nearestDistanceKm,
                  ),
                );
  ...
  Future<List<VisitedHeritageSite>> loadByCountry(String countryCode) async {
    final rows =
        await (_db.select(_db.visitedHeritageSites)
