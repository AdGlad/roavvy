...
class ArtworkConfirmationService {
  ...
  Future<String> create(ArtworkConfirmation confirmation) async {
    await _collection(
      confirmation.userId,
    ).doc(confirmation.confirmationId).set(confirmation.toFirestore());
  ...
  Future<void> archive(String uid, String confirmationId) => _collection(uid)
      .doc(confirmationId)
      .update({'status': ArtworkConfirmationStatus.archived.firestoreValue});
}
