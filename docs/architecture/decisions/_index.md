...
| 152 | M102 Achievement-Aware Merch Context: Continent/Region/Passport Scope | Accepted | `Achievement` gains `continentScope`/`regionScope` fields; `kCountrySubRegion` maps alpha-2 → sub-region key; `AchievementEngine` computes per-continent and per-region counts + passport stamp milestones; `MerchContext._resolveCodes` filters by scope; `buildItems()` dispatches to `_buildContinentExplorerItems`, `_buildRegionItems`, `_buildPassportMilestoneItems` |
| 153 | M103 Expanded Merch Templates: Typography + Badge + Shirt Colour Intelligence | Accepted | `CardTemplateType.typography` (stacked country-name text composition) and `CardTemplateType.badge` (circular explorer badge) added; `TypographyCard` and `BadgeCard` CustomPaint widgets in `card_templates.dart`; `merchSuggestShirtColor()` pure function; `PulseMerchOption.suggestedShirtColor` field; `LocalMockupPreviewScreen.initialColour` param; `MerchContext` builders include new templates per achievement type; badge skipped when `codes.length > 15` |
| 154 | M104 Intelligent Merch Recommendation Engine: Density-Aware Ranked Templates | Accepted | `MerchDensityClass` enum (solo/small/medium/large/massive) classifies country/stamp count; `MerchTemplateRanker.rankFor()` pure function returns ranked+excluded template list by density + achievement type; `MerchStory.forOption()` generates contextual title+subtitle per template/achievement/density; `MerchContext.buildItems()` replaced with `_buildFromRankedTemplates()` helper capped at `maxForDensity(density)` non-excluded templates; `PulseMerchOption.contextLabel` field shown as third line in `MerchOptionCard`; `_addGroup` gains optional `contextLabel` param; `merchSuggestShirtColor` gains optional `density` param; `merchAutoTuneCodes`/`merchAutoTuneStamps` updated to 5-tier model |
| 155 | Six-layer automated testing architecture | Accepted | Unit → Service → Widget → Integration → Emulator → Device; each layer has defined purpose, tools, and run frequency |
| 156 | Standardised testing technology stack | Accepted | `flutter_test` + `mocktail` + `fake_cloud_firestore` + `firebase_auth_mocks` + `integration_test` + `lcov` + GitHub Actions; no alternatives without new ADR |
| 157 | GitHub Actions CI quality gates on every pull request | Accepted | 7-step workflow: deps → analyze → format → unit → service → widget → coverage; no merge if any step fails |
| 158 | Coverage philosophy: confidence over percentage | Accepted | Targets exist to guide prioritisation; business logic 85%→90%, revenue workflows 80%→90%, overall 50–65%→70%; high-risk areas prioritised over raw coverage |
| 159 | Incremental test-and-fix discipline | Accepted | Small area → add tests → run → classify failures → fix → rerun → commit; tests must not be weakened; no milestone complete with unresolved failures |
