...
- **Dual-Placement T-Shirt Print & Mockup (M63 — ADR-120)**:
  - `types.ts`: `MerchConfig` gains 8 dual-placement fields (`frontPrintFileStoragePath`, `frontPrintFileSignedUrl`, `frontPrintFileExpiresAt`, `backPrintFileStoragePath`, `backPrintFileSignedUrl`, `backPrintFileExpiresAt`, `frontMockupUrl`, `backMockupUrl`); deprecated single-file fields made optional (`?`); `CreateMerchCartRequest` replaces `placement` + `clientCardBase64` with `frontCardBase64` + `backCardBase64` (legacy `clientCardBase64` aliased to `backCardBase64`); `CreateMerchCartResponse` gains `frontMockupUrl` + `backMockupUrl`
  - `generatePrintfulMockup()` replaced by `generateDualPlacementMockups(variantId, frontUrl, backUrl)` — single Printful mockup task with both `front` and `back` placements; polls same 10×2s window; returns `{ frontMockupUrl, backMockupUrl }`
  - `createMerchCart`: generates `print_files/{id}_back.png` (full back card) and `print_files/{id}_front.png` (front ribbon composited onto transparent 4500×5400 canvas at left-chest offset: x=750px, y=900px, 600px wide via `sharp.composite`); both files uploaded with 7-day signed URLs; deprecated single-file fields kept pointing to back file for backward compat; returns both mockup URLs
  - `shopifyOrderCreated`: detects new dual-file orders via `config.frontPrintFileStoragePath !== null`; submits `files: [{url: front, type: 'front'}, {url: back, type: 'back'}]`; guard added for null `frontPrintFileSignedUrl`; falls back to `type: 'default'` for old single-file orders
  - `LocalMockupPreviewScreen`: stores `_frontRibbonBytes: Uint8List?` from `CardRenderResult.bytes` alongside `_frontRibbonImage`; `_onApprove()` sends `frontCardBase64` + `backCardBase64`; removes `placement` from payload; `_mockupUrl` replaced by `_frontMockupUrl`, `_backMockupUrl`, `_showingFront`; `_onFlipped` now updates `_showingFront`; ready-state renders `_showingFront ? _frontMockupUrl : _backMockupUrl`, falling back to local mockup when null
...
| M61 | Commerce / Visual Quality — Passport Card Refinement: Safe zones for Title (top 18%) and Branding (bottom-left), integrated text rendering in CustomPainter (no underlines), pure white / transparent background support, user customization for stamp/date colors and title text, unified single-image rendering across all screens. (ADR-117) | ✅ Complete |
| M67 | Visual Design — Grid Card Upgrade: `GridFlagsCard` and `HeartFlagsCard` converted to `StatefulWidget`; `ValueNotifier`-based async SVG preloading via `FlagTileRenderer.loadSvgToCache()` — flags now render real SVG images after async load, emoji fallback on first frame; `_HeartPainter` wired with `titleOverride` and `_drawTitle()` (drawn before heart clip); `CardImageRenderer._cardWidget()` passes `titleOverride` to Grid and Heart. (ADR-123) | ✅ Complete |
