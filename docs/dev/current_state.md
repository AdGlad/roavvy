# Roavvy — Development State (as of 2026-03-26, through Task 155 / M44)
...
- **Passport Stamp Card: Authentic Stamp Renderer (M44 — Tasks 151–155, ADR-096)**:
  - `StampData` immutable model + `StampShape` enum (circular, rectangular, oval, doubleRing) + `StampColor` enum (5 ink colours) in `passport_stamp_model.dart`; `StampData.fromTrip()` and `StampData.fromCode()` factories; `dateLabel` formatted as "DD MMM YYYY" via `intl`
  - `StampPainter` (`lib/features/cards/stamp_painter.dart`) — `CustomPainter` drawing one stamp; `canvas.save/translate/rotate/restore` for correct rotation; shape anatomy: circular (concentric circles + country code + date + ENTRY/EXIT), rectangular (double-border rect), oval (RRect border), doubleRing (two circles + 24 serrated ticks); all text drawn via `TextPainter` centred at offset; `shouldRepaint` compares stamp identity
  - `PassportLayoutEngine.layout()` (`lib/features/cards/passport_layout_engine.dart`) — deterministic seeded positioning; `seed` defaults to `countryCodes.join().hashCode`; trips sorted by `startedOn` first, then bare codes; up to 20 stamps; 8-placement-attempt collision avoidance (80% combined radius threshold); shape cycles by index; color from 2-char country code hash; rotation ±12°; scale 0.85–1.15
  - `PaperTexturePainter` (`lib/features/cards/paper_texture_painter.dart`) — procedural parchment: warm base `0xFFF5ECD7`; ~1000 seeded micro-rects grain; two faint horizontal fold lines at 30%/70%; radial gradient corner aging; `shouldRepaint` always false (static)
  - `PassportStampsCard` upgraded to `CustomPainter` pipeline: `PaperTexturePainter` background + `_MultiStampPainter` (iterates `StampPainter` for each stamp) + `ROAVVY` watermark; `LayoutBuilder` provides canvas `Size` to `PassportLayoutEngine.layout()`; accepts optional `trips: List<TripRecord>` for real date/ENTRY labels; `CardGeneratorScreen` passes `trips` from `tripListProvider`
  - Pre-existing `country_region_map_screen_test.dart` failure fixed: `polygonsProvider.overrideWithValue(const [])` added to `_pumpScreen` helper (was triggering `geodataBytesProvider not overridden` in `_fitBounds`)
  - 536 flutter tests passing (no regressions)
