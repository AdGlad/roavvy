...
FORCE_MULTI = {'us', 'gb', 'jp', 'gr', 'id', 'ph', 'nz', 'fj', 'ca', 'no'}
...
def select_polygons(mp: MultiPolygon, iso2: str, force_mainland: bool, force_multi: bool, bbox_filter: dict) -> MultiPolygon:
    ...
    # Apply bbox filter first (e.g. France: remove DOM/TOM).
    if bbox_filter:
        lon_max = bbox_filter.get('lon_max')
        if lon_max is not None:
            polys = [p for p in polys if p.centroid.x < lon_max]
        if not polys:
            polys = list(mp.geoms)  # fallback to original if filter too aggressive

    if force_mainland or (not force_multi and iso2 not in FORCE_MULTI):
        # Use only largest polygon if it covers ≥ 80% of total area.
        total_area = sum(p.area for p in polys)
        if total_area > 0:
            largest = max(polys, key=lambda p: p.area)
            if largest.area / total_area >= 0.80 or force_mainland:
...
def normalise(mp: MultiPolygon) -> tuple[float, float, list]:
    ...
    """
    bounds = mp.bounds  # (minx, miny, maxx, maxy)
